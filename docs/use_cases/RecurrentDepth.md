...
Stage 1 worker–analyzer pipeline
...
```mermaid
sequenceDiagram
  participant Loop as iterate_forward
  participant W as RecurrentDepthWorker
  participant A as RecurrentConvergenceAnalyzer
  participant C as ExitController

  Loop->>Loop: core_block_forward step t
  Loop->>W: build_state(x_t, x_t-1, t)
  Note over W: hidden_delta, h_norm, rel_delta on GPU
  W-->>Loop: ConvergenceState [B,S] scalars
  Loop->>A: analyze(state, ctrl)
  A->>C: update prev_delta / nonconverging
  A-->>Loop: AnalyzerDecision exit_mask, steer_gate
  Loop->>C: apply(decision) → active mask
  Loop->>Loop: freeze exited rows; optional decode row-slice
```
