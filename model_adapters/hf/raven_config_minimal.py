"""HuggingFace Raven/Huginn ``PretrainedConfig`` (HF oracle only).

``AdaptiveRavenForvLLM`` does **not** use this file. The vLLM executor uses
``model_adapters.vllm.original_raven_vllm.RavenConfig`` plus checkpoint
``config.json`` via HookLLM. This module exists because upstream
``raven_modeling_minimal_llama.py`` does ``from .raven_config_minimal import RavenConfig``.
"""
