# type: ignore
"""
raven_modeling_minimal_llama.py

Modeling file for HF compatibility and zero-shot experiments.

This module contains the model architecture and class definitions for the
retrofitted recurrent depth Raven models, as proposed by McLeish et al. (2025).

Source:
    Repository: https://github.com/mcleish7/retrofitting-recurrence/blob/master/
    Paper: "Teaching Pretrained Language Models to Think Deeper with Retrofitted Recurrence",
           McLeish et al., 2025 (https://arxiv.org/pdf/2511.07384)
    Models: https://hf.co/collections/tomg-group-umd/retrofitting-recurrence
    License: Apache 2.0
...
class RavenPreTrainedModel(PreTrainedModel):
    ...
    # transformers≥5 expects a {target: source} map (not a list of targets).
    ...
    def _init_weights(self, module):
        # Raven checkpoints are loaded from pretrained weights; random init is unused.
        return
...
class RavenForCausalLM(RavenPreTrainedModel, GenerationMixin):
    ...
    def __init__(
        ...
        # transformers≥5: PreTrainedModel.__init__ no longer calls post_init(); it
        # registers all_tied_weights_keys and runs weight init / tying.
        self.post_init()
    ...
    def forward(
        ...
        **kwargs,
        ...
        # transformers≥5 `generate` may pass 3D input_ids (e.g. [B, 1, S]) and
        # 1D/2D cache_position; Raven blocks expect [B, S, E] latents.
        ...
        if position_ids is None and cache_position is None:
            seq = input_ids.shape[1] if input_ids is not None else input_embeds.shape[1]  # type: ignore[union-attr]
            position_ids = torch.arange(seq, device=self.device).unsqueeze(0)
        elif cache_position is not None:
            position_ids = cache_position.unsqueeze(0) if cache_position.ndim == 1 else cache_position
        if position_ids is not None and position_ids.ndim == 3 and position_ids.shape[0] == 1:
