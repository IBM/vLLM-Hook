...
# VLLM-SPECIFIC TARGET MATCHING:
# This file identifies attention modules by vLLM/PyTorch-style module names.
ATTN_PATTERNS = [
...
# HOOK CORE:
# The wrapper is the actual interception mechanism. It works by replacing a
# live child module in the model tree, not by using a native MLX hook API.
class MLXHookWrapper(nn.Module): 
    ...
    def __call__(self, x, *args, **kwargs): 
        # HOOK PAYLOAD ENTRY:
        # Observe the call before delegating to the original module.
        self.hook_fn((x,), self.name)
        
        # HOOK CORE:
        # Preserve model behavior by forwarding into the original module.
...
class ProbeHookQKWorker(V1Worker):
    ...
    def _install_hooks(self):
        ...
        # VLLM-SPECIFIC RUNTIME SETUP:
        # These variables control when hooks are active and where captures are
        # written. This is separate from the hook transport mechanism.
        ...
        # HOOK PAYLOAD LOGIC:
        # This defines what to capture once a wrapped attention module is called.
        ...
        # HOOK INSTALLATION:
        # Discover target modules, walk to each parent object, and replace the
        # child module in place with a wrapper.
        self._hooks = []
        matched = []
        for name, module in model.named_modules():
            ...
            # BROKEN / PLACEHOLDER:
            # The commented code below is the original PyTorch-style
            # register_forward_hook idea. The wrapper replacement below is the
            # actual interception path in this sketch.
            ...
            # HOOK CORE:
            # Replace parent.<target_name> with the wrapper so future forward
            # calls enter MLXHookWrapper.__call__ first.
            target_name = parts[-1]
            setattr(parent, target_name, wrapped_module)
            
            # BROKEN / PLACEHOLDER:
            # There is no hook handle object here because register_forward_hook
