...
def test_registries_are_frozen_and_closed():
    assert kinds.TRANSFORM_KINDS == frozenset(
        {"additive", "projection", "rotation", "head_additive"}
    )
    ...
    assert kinds.READOUT_KINDS == frozenset({"affine", "cosine", "projected_cosine"})
    assert kinds.RULE_KINDS == frozenset({"per_key_threshold", "sum_threshold"})
...
def test_kind_params_covers_every_parameterized_kind():
    ...
    assert kinds.KIND_PARAMS["projection"] == {}
    ...
    assert kinds.KIND_PARAMS["affine"] == {}
    assert kinds.KIND_PARAMS["cosine"] == {}
    assert kinds.KIND_PARAMS["projected_cosine"] == {}
    assert kinds.KIND_PARAMS["sum_threshold"] == {"bias": float}
    assert kinds.KIND_PARAMS["per_key_threshold"] == {
        "threshold": float,
        "comparator": str,
        "aggregate": str,
    }
...
def test_artifact_tensors_and_constraints_name_registered_kinds():
    ...
    for kind in kinds.CONSTRAINTS:
        assert kind in all_kinds
    assert kinds.ARTIFACT_TENSORS["projection"] == ("vector",)
    assert kinds.ARTIFACT_TENSORS["affine"] == ("weights",)
    assert kinds.ARTIFACT_TENSORS["cosine"] == ("directions",)
    assert kinds.ARTIFACT_TENSORS["projected_cosine"] == ("directions",)
